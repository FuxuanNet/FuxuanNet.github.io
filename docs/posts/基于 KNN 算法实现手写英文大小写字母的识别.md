...
|:----------:|:---:|:----------:|----------------|
| - | - | 数据加载与预处理模块 | 负责矩阵加载、图像向量化及标签创建。 |
| - | - | 距离计算与排序模块  | 负责计算欧几里得距离，完成排序与最近邻提取。  |
| - | - | 图形界面与结果显示  | 负责界面设计、文件选择功能、结果显示与最终集成测试。 |
...
- **设计思想**：
  ...
  $$
  1. 将所有距离存储到 `distances` 数组中，并调用 `qsort` 函数升序排序。
  2. 提取距离最小的 K 个样本的标签，并存储到 `result` 数组。
  3. 确保模块对所有样本的距离计算高效、准确。
- 代码行数：
- 实验代码及注释：
